...
# fct_infreq(): reorder factor levels by number of observations with each level (largest first)
...
Find counts of lizard by species, reorder factor levels by count, update aesthetic by color (removing redundant legend), customize color using something from `paletteer`:
...
There are a lot of options for colors in R. Color schemes can be very important for figures and are also a great way to have some fun.

Here are some useful resources that may help:

1. The **ggsci** R package contains a collection of high-quality color palettes inspired by colors used in scientific journals, data visualization libraries, and more.
2. There are even color palettes inspired by Beyonce and Wes Andersen movies.
3. Other great resources include [`Color Hexa`](colorhex.com) and the [`colortools package`](http://www.sthda.com/english/wiki/the-elements-of-choosing-colors-for-great-data-visualization-in-r)
...
```
## Add in some graphics if you want to be really fancy

The [`rphylopic`](https://jacintak.github.io/post/2021-08-01-rphylopic/) package is an easy way to add in some vector images.
...
```{r}
unique(lizards$scientific_name)
```

```{r}
lizard_pic <- name_search(text = "Cnemidophorus", options = "namebankID")[[1]] # find names
lizard_pic
```
```{r}
lizard_id_all <- name_images(uuid = lizard_pic$uid[1])  # list images
lizard_id_all
```

```{r}
lizard_id <- name_images(uuid = lizard_pic$uid[1])$supertaxa[[1]]$uid  # get individual image id
lizard_id
```

```{r}
lizard_image <- image_data(lizard_id, size = 256)[[1]] # get actual icon, define size. Don't run this alone
```


```{r}
p+add_phylopic(lizard_image, alpha = 1, x = 375, y = 50, ysize = 25) +
  theme_light()
```

Note that you can also load in ['IAN symbol library'](https://twitter.com/F_J_Heather/status/1455333432523849730/photo/1) graphics and use them in R plots
...
```{r}
lake_plot<-ggplot(data = mono, aes(x = year, y = lake_level)) +
...
```{r}
shrimp <- name_search(text = "Shrimp", options = "namebankID")[[1]] # find names
shrimp_id_all <- name_images(uuid = shrimp$uid[1])  # list images
shrimp_id <- name_images(uuid = shrimp$uid[1])$same[[1]]$uid  # get individual image id
shrimp_image<-image_data(shrimp_id, size = 256)[[1]] # get actual icon, define size. Don't run this alone
```


```{r}
lake_plot + add_phylopic(shrimp_image, alpha = 1, x = 1875, y = 6365, ysize = 10)
```
...
two_lizards_plot<-ggplot(data = two_lizards, aes(x = total_length, y = weight)) +
  geom_point(aes(color = common_name,
                 shape = common_name),
             size = 2) +
...
You can move the legend inside of the figure, if appropriate
...
Let's look at what our plot was before

```{r}
p
```

Let's highlight toe marker number value equal to 250
...
Now let's highlight two of the sites
...
```{r}
p+q+plot_annotation(tag_levels = "A")
```
...
((p | q) / z) + plot_annotation(tag_levels = "a") & theme_dark() 
```
...
```{r, eval = FALSE}
ggsave(filename = "file_name.png", width = 6, height = 4, dpi = 300)
```

# Learn more

Here are some great resources for building advanced data visualization tools in `ggplot2`, learning about new types of graphs, and choosing an appropriate graph type: 
